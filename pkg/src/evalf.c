...
/* this function is used to evaluate f if not all a_i are -Inf. */
...
{
    double res = evalf(INTEGER(n)[0], INTEGER(q)[0], REAL(U), REAL(a), REAL(b), REAL(C), REAL(nu)[0], REAL(ONE)[0], REAL(ZERO)[0]);
